...
class ServiceManager:
    ...
    def __init__(self, socat_port: int = 8081, sleep_duration: int = 3) -> None:
        ...
        self.services: Dict[str, int] = {
            'here_is_paw_admin_1': 8082,
            'here_is_paw_admin_2': 8083
        }
    ...
    def _find_current_service(self) -> None:
        ...
        if not current_service:
            self.current_name, self.current_port = 'here_is_paw_admin_2', self.services['here_is_paw_admin_2']
        else:
    ...
    def _cleanup_dangling_images(self) -> None:
        ...
        # 먼저 이미지 목록 확인
        cmd_list = "docker images | grep '.*/here-is-paw/here_is_paw_admin' | grep '<none>'"
        ...
        # 이미지 정리
        cmd_remove = "docker images | grep '.*/here-is-paw/here_is_paw_admin' | grep '<none>' | awk '{print $3}' | xargs -r docker rmi"
    ...
    def _run_container(self, name: str, port: int) -> None:
        os.system(
            f"docker run -d --name={name} --restart unless-stopped -p {port}:8081 -e TZ=Asia/Seoul -v /dockerProjects/here_is_paw_msa/admin/volumes/gen:/gen --pull always ghcr.io/here-is-paw/here_is_paw_admin")
    ##
